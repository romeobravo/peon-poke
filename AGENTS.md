...
All logic — dispatch, adapters, setup, uninstall, doctor — lives in the Python CLI `peon-poke` (python3.8+, stdlib only, tomllib used opportunistically). The `adapters/*.sh` entry points are 2–5 line `exec` shims with zero logic, kept for hook wiring and muscle memory. Claude hooks are registered as `…/bin/peon-poke dispatch <category>` (shlex-joined); Codex as `notify = ["…/bin/peon-poke", "codex"]`. Ownership is exactly these shapes — no legacy shapes are recognized. The `peon-poke` PATH symlink points at the CLI file. `peon-poke setup [agent]` registers a single integration (`claude`, `codex`, `pi`, `oh-my-pi`, `opencode`) or, bare, all detected ones.
...
- Config schema changes must be backward-tolerant: the CLI treats `custom`, `strength`, and `enabled` as optional because users own `~/.config/peon-poke/config.json` and installs never overwrite it.
...
- `install-remote.sh` verifies every fetched file against `SHA256SUMS` **before executing anything**, and hard-fails if the manifest is missing at the install base. The manifest also drives the fetch list — it must include the universal dist binary and the `peon-poke` CLI (setup installs it as `~/.peon-poke/bin/peon-poke`). Every manifest entry must pass `valid_payload_path`: no absolute paths, `.`/`..` components, backslashes, odd characters, or top-level names outside the hard-coded payload set (`install.sh`, `peon-poke`, `config.json`, `dist/`, `plugins/`, `adapters/`). Adding a new top-level fetched file means updating that allowlist **and** `scripts/sha256sums.sh` in the same commit.
- `peon-poke setup` never writes through symlinks: `~/.claude/settings.json`, installed extension/plugin files, and an existing `peon-poke` PATH entry that is a symlink is left untouched (a symlink is only ever refreshed when it points exactly at our own target). Claude hook ownership is exact — precisely the shape setup writes (`…/bin/peon-poke dispatch <category>`) — so a user's own hook that merely mentions peon-poke survives both setup and uninstall.
- `peon-poke uninstall` restores a user-customized extension from its `.peon-poke-bak` instead of deleting it — their edits are theirs.
...
`install-remote.sh` fetches exactly the files listed in `SHA256SUMS` and verifies each one. Whenever you touch `install.sh`, `peon-poke`, `config.json`, `dist/poke-darwin-universal`, `plugins/pi/*.ts`, or `adapters/*.sh`, regenerate and commit the manifest:
